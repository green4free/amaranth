    ...
    def emit_submodules(self):
        for submodule_idx in self.module.submodules:
            ...
            if not self.empty_checker.is_empty(submodule_idx):
                ...
                if submodule._p_signature is not None and submodule._p_signature in _ir.Fragment.pureFragmentCache:
                    t = _ir.Fragment.pureFragmentCache[submodule._p_signature]
                    if t[1] >= 0 and t[1] != submodule_idx:
                        module_name = t[2]
                dotted_name = ".".join(module_name)
                ports = {}
...
def convert_fragment(fragment, ports=(), name="top", *, emit_src=True, **kwargs):
    ...
    for module_idx, module in enumerate(netlist.modules):
        if empty_checker.is_empty(module_idx):
            continue
        if module._p_signature is not None:
            t = _ir.Fragment.pureFragmentCache.get(module._p_signature, (None,-1))
            if t[1] >= 0 and t[1] != module_idx:
                continue
