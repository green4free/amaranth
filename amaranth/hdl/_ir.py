...
class Fragment:
    ...
    def add_p_signature(self, p_signature):
        self._p_signature = p_signature
        if p_signature is not None and p_signature not in Fragment.pureFragmentCache:
            Fragment.pureFragmentCache[p_signature] = (self, -1)
...
class NetlistEmitter:
    ...
    def emit_fragment(self, fragment: _ir.Fragment, parent_module_idx: 'int | None', *, cell_src_loc=None):
        ...
        if isinstance(fragment, _ir.Instance):
            ...
            module_idx = self.netlist.add_module(parent_module_idx, fragment_name, src_loc=fragment.src_loc, cell_src_loc=cell_src_loc, p_signature=fragment._p_signature)
            if fragment._p_signature is not None:
                t = _ir.Fragment.pureFragmentCache[fragment._p_signature]
                if t[1] < 0:
                    _ir.Fragment.pureFragmentCache[fragment._p_signature] = (t[0], module_idx, fragment_name)
